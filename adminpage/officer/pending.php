...
if (isset($_GET['id'])) {
    ...
    $personID = mysqli_real_escape_string($link, $_GET['id']);

    // Update the status to 'approved' in the database
    $sql = "UPDATE Person SET Status = 'approved' WHERE PersonID = '$personID'";
    if (mysqli_query($link, $sql)) {
        // Redirect back to the pending.php page
        ...
        <div class="col-md-12">
            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    $sql = "SELECT * FROM Person WHERE Status = 'pending'";
                    if ($result = mysqli_query($link, $sql)) {
                        if (mysqli_num_rows($result) > 0) {
                            while ($row = mysqli_fetch_array($result)) {
                                ...
                                echo '<a href="action_approve.php?id=' . $row['username'] . '" class="btn btn-success btn-action">Approve</a>';
                                echo '<a href="reject_action.php?id=' . $row['username'] . '" class="btn btn-danger btn-action ml-2">Reject</a>';
                                echo '<a href="view_action.php?id=' . $row['username'] . '" class="btn btn-info btn-action ml-2">View</a>';

                                // Display "ordinary" or "express" based on the amount
                                $amount = $row['amount'];
                                if ($amount == 320) {
                                    echo '<span class="application-type">Ordinary</span>';
                                } elseif ($amount == 5200) {
                                    echo '<span class="application-type">Express</span>';
                                }
                            ...
                            echo '<tr><td colspan="9">No pending applications found.</td></tr>';
                        }
                    } else {
